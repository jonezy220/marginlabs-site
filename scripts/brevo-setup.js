 ...
 *
 * Safe to re-run. Upserts templates: updates existing by name, creates missing.
 * Attributes and lists still skip-if-exists (no copy in them to refresh).
 */
...
const EMAILS = [
  ...
  },
  {
    ...
    cta: ['Get the Strategic Framework, $139', 'https://marginlabs.io/#products'],
    body: [
  ...
  },
  {
    ...
    body: [
      P('If embedded payments is on your roadmap, even loosely, the <a href="https://marginlabs.io/#products" style="color:#C8823C; text-decoration:underline;">Strategic Decision Framework</a> ($139) is the most efficient way to understand the model economics and what execution requires at your stage.'),
...
async function createTemplates() {
  // Fetch existing templates so we can upsert by name. Brevo's API matches
  // templates by ID, so we map name -> ID once and then update or create
  // per email as appropriate.
  ...
  for (const email of EMAILS) {
    ...
    const existingId = existingByName.get(email.name);

    if (existingId) {
      // Update existing template by ID. Brevo PUT returns 204 No Content.
      const { status, ok, data } = await brevo('PUT', `/smtp/templates/${existingId}`, payload);
      if (ok || status === 204) {
        console.log(`  ↻ Updated template: ${email.name} (ID: ${existingId})`);
      } else {
        console.log(`  ✗ Update failed for ${email.name}: ${JSON.stringify(data)}`);
      }
    } else {
      // Create new template. POST returns 201.
      const { status, data } = await brevo('POST', '/smtp/templates', payload);
      if (status === 201) {
